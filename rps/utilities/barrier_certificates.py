...
class SIRealtimeDeltaBarrierCertificate(BarrierCertificate):
    """
    The SI Realtime Delta Barrier Certificate is a delta barrier certificate that updates based on real elapsed
    time.  The SIDeltaBarrierCertificate uses sim time (i.e. 0.033 seconds) per update, while the realtime
    barrier certificate uses python's time library.
    """
    def __init__(
        self,
        barrier_gain = 100,
        ...
        delta_der = lambda t: np.pi / 4 * np.sin(np.pi * 2 / 2) if t <= 2 else 0,
        obstacle_gain = 10,
        obstacles = []
    ):
        super().__init__(barrier_gain, magnitude_limit, obstacle_gain, obstacles)
...
class UniBarrierCertificate(BarrierCertificate):
    """
    A Barrier certificate for unicycle dynamics.
    """
    def __init__(
        self,
        si_barrier_certificate: BarrierCertificate,
        projection_distance = 0.12,
    ):
        self.si_barrier = si_barrier_certificate
        self.si_to_uni_dyn, self.uni_to_si_states = create_si_to_uni_mapping(projection_distance=projection_distance)
        self.uni_to_si_dyn = create_uni_to_si_dynamics(projection_distance=projection_distance)

    def name(self) -> str:
        return f"unicycle({self.si_barrier.name()})"
    
    def h(self, x1: np.ndarray, x2: np.ndarray) -> float:
        x1_si = self.uni_to_si_states(x1)
        x2_si = self.uni_to_si_states(x2)
        return self.si_barrier.h(x1_si, x2_si)
    
    def dh(self, x1: np.ndarray, x2: np.ndarray) -> np.ndarray:
        x1_si = self.uni_to_si_states(x1)
        x2_si = self.uni_to_si_states(x2)
        dh_si = self.si_barrier.dh(x1_si, x2_si)
        return dh_si
    
    def apply(self, dxu: np.ndarray, x: np.ndarray) -> tuple[np.ndarray, float]:
        x_si = self.uni_to_si_states(x)
        dxi = self.uni_to_si_dyn(dxu, x)
        dxi, h_min = self.si_barrier(dxi, x_si)
        return self.si_to_uni_dyn(dxi, x), h_min
